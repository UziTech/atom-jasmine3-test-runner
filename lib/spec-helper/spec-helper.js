...
export default async function (options, setupView) {
	...
	const install = async function (pkg, version) {
		...
		if (version) {
			const pkgPath = await installDep(pkg, version);
			require(pkgPath);
		} else {
			require(pkg);
		}
	...
	try {
		...
		if (all || options.jasmineJson) {
			await install("jasmine2-json", "^1.1.1");
		}
		...
		if (all || options.jasmineShouldFail) {
			await install("jasmine-should-fail", "^1.1.7");
		}

		if (all || options.jasmineTagged) {
			await install("jasmine2-tagged", "^1.1.1");
		}

		if (all || options.jasmineFocused) {
			await install("jasmine2-focused", "^1.1.1");
		}
		...
		if (all || options.mockLocalStorage) {
			await install("jasmine-local-storage", "^1.1.1");
		}
