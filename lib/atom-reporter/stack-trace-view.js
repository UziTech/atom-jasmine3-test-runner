...
export default class StackTraceView {
	...
	render() {
		...
		let jasminePattern = /^\s*at\s+(<Jasmine>|.*\(?.*[/\\]jasmine-core.*[/\\]jasmine(-[^/\\]*)?\.js:\d+:\d+\)?)\s*$/;
		let lines = [];
		...
		return (
			<pre className={ `j3-stack-trace padded ${this.props.expanded ? "expanded" : ""}`} on={{ click: this.onClick }}>{ lines }</pre>
		);
	}
