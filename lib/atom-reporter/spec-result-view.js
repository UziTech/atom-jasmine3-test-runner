...
export default class SpecResultView {
	...
	render() {
		const shouldFail = this.props.spec.shouldFail && this.props.spec.status === "passed";
		let passedExpectations = [];
		if (shouldFail) {
			passedExpectations = this.props.spec.passedExpectations;
		}

		return (
			<div id={`spec-failure-${this.props.spec.id}`} className={`spec ${shouldFail ? "should-fail" : ""}`} >
				<div className={`j3-spec-toggle ${this.props.folded ? "icon-unfold" : "icon-fold"}`} on={{click: this.onToggle}}></div>
				...
				<div className={`spec-failures ${this.props.folded ? "hidden" : ""}`}>
					...
					{passedExpectations.map(expectation =>
						<ExpectationView expectation={expectation} specDirectory={this.props.spec.specDirectory} />
					)}
					{this.props.spec.failedExpectations.map(expectation =>
						<ExpectationView expectation={expectation} specDirectory={this.props.spec.specDirectory} />
					)}
				</div>
			</div>
